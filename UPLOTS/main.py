...
#     print('Avalable instruct_list: ',results) 
    ...
    for config_item in configfile:
        ...
        config['dataloader']['train_dataset']['params']['window']=args.seq_len
        config['dataloader']['test_dataset']['params']['window']=args.seq_len
        config['model']['params']['seq_length']=args.seq_len
    ...
    if args.train:
        ...
        print('num is: ',len(dataset))
        samples = trainer.sample(instruct=ins[0] ,num=len(dataset)*dataset.var_num, size_every=900, shape=[dataset.window, 1])
        if dataset.auto_norm:
            samples = unnormalize_to_zero_to_one(samples)
            # samples = dataset.scaler.inverse_transform(samples.reshape(-1, samples.shape[-1])).reshape(samples.shape)
        np.save(os.path.join(args.save_dir, f'ddpm_fake_{args.config_file[0]}_milestone_{args.milestone}_mask{args.mask_rate}_len{args.seq_len}.npy'), samples)
