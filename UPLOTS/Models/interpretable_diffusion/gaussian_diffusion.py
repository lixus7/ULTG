...
#         self.gpt2.h = self.gpt2.h[:6] # gpt_layer = 6
...
#         self.gpt2.train()
        ...
        # print('fast_sampling yes? ',self.fast_sampling) # false
    ...
    def output(self, instruct,  x, t, mask=None,padding_masks=None):
        trend, season = self.model(instruct, x, t, mask, padding_masks=padding_masks)
        model_output = trend + season
        
        return model_output

    def model_predictions(self, instruct, x, t, clip_x_start=False, mask=None, padding_masks=None):
        if padding_masks is None:
            padding_masks = torch.ones(x.shape[0], self.seq_length, dtype=bool, device=x.device)

        maybe_clip = partial(torch.clamp, min=-1., max=1.) if clip_x_start else identity
        x_start = self.output(instruct, x, t, mask, padding_masks)
        x_start = maybe_clip(x_start)
        pred_noise = self.predict_noise_from_start(x, t, x_start)
        return pred_noise, x_start

    def p_mean_variance(self, instruct, x, t, clip_denoised=True):
        _, x_start = self.model_predictions(instruct, x, t)
        if clip_denoised:
            x_start.clamp_(-1., 1.)
    ...
    def p_sample(self, instruct , x, t: int, clip_denoised=True):
        batched_times = torch.full((x.shape[0],), t, device=x.device, dtype=torch.long)
        model_mean, _, model_log_variance, x_start = \
            self.p_mean_variance(instruct, x=x, t=batched_times, clip_denoised=clip_denoised)
        noise = torch.randn_like(x) if t > 0 else 0.  # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def sample(self, instruct, shape):
        # print('fast shape is ', shape)
        device = self.betas.device
        img = torch.randn(shape, device=device)
        for t in tqdm(reversed(range(0, self.num_timesteps)),
                      desc='sampling loop time step', total=self.num_timesteps):
            img, _ = self.p_sample(instruct, img, t)
            
        return img

    @torch.no_grad()
    def fast_sample(self, instruct, shape, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(instruct, img, time_cond, clip_x_start=clip_denoised)
    ...
    def generate_mts(self, instruct, batch_size=16):
        feature_size, seq_length = self.feature_size, self.seq_length
        sample_fn = self.fast_sample if self.fast_sampling else self.sample
        return sample_fn(instruct, (batch_size, seq_length, feature_size))
    ...
    def _train_loss(self, instruct, x_start, t, mask=None, target=None, noise=None, padding_masks=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        if target is None:
            target = x_start

        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # noise sample
        model_out = self.output(instruct, x, t, mask, padding_masks)
        # print('model_output shape is : ', model_out.shape)  # torch.Size([128, 24, 7]
        # exit()
    ...
    def forward(self, instruct, x, **kwargs):
        b, c, n, device, feature_size, = *x.shape, x.device, self.feature_size
        # print('x shape is :', x.shape)  # torch.Size([128, 24, 7]
        # assert n == feature_size, f'number of variable must be {feature_size}'
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self._train_loss(instruct, x_start=x, t=t, **kwargs)
    ...
    def sample_infill(
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)),
            ...
            img = self.p_sample_infill(instruct, x=img, t=t, clip_denoised=clip_denoised, target=target,
    ...
    def p_sample_infill(
        ...
        pred_img = self.langevin_fn(ins, sample=pred_img, mean=model_mean, sigma=sigma, t=batched_times,
                                    tgt_embs=target, partial_mask=partial_mask, **model_kwargs)
    ...
    def langevin_fn(
        ...
        with torch.enable_grad():
            for i in range(K):
                ...
                x_start = self.output(instruct, x=input_embs_param, t=t)
