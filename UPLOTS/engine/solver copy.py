...
#         self.dl = cycle(dataloader['dataloader'])
    ...
    def train(self):
        ...
        # 改这里    
        import math
        weak_dataset_idx = self.args.weak_idx   # “第二个”数据集的索引
        alpha = self.args.alpha  # 加权系数，可自行调节
        with tqdm(initial=step, total=self.max_train_batches*len(self.train_loaders)*self.args.epoch/2) as pbar:
                ...
                while True:
                    ...
                    for _ in range(self.gradient_accumulate_every):
                        try:
                            ...
                            data = torch.reshape(data.permute(0, 2, 1),(b*n,t,1)) 
                            mask = torch.rand((b*n, t, 1)).to(self.device)
