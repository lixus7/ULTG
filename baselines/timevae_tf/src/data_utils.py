...
def load_data(seq_len, data_dir: str, dataset: str) -> np.ndarray:
    """
    ...
    if dataset=="etth" :
        print('load etth')
        #  filepath = os.path.join(data_dir, f"ETTh.csv")
        df = pd.read_csv(data_dir + '/ETTh.csv', header=0)
    ...
    for i in range(data.shape[0]-seq_len+1):
        data_single.append(data[i:i+seq_len])
    all_data = np.array(data_single)
