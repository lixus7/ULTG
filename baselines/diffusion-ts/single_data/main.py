...
def main():
    ...
    if args.train:
        ...
        # samples = trainer.sample(num=len(dataset), size_every=900, shape=[dataset.window, dataset.var_num])
        samples = trainer.sample(num=len(dataset)*dataset.var_num, size_every=900, shape=[dataset.window, 1])
        if dataset.auto_norm:
            samples = unnormalize_to_zero_to_one(samples)
            # samples = dataset.scaler.inverse_transform(samples.reshape(-1, samples.shape[-1])).reshape(samples.shape)
        np.save(os.path.join(args.save_dir, f'ddpm_fake_{args.name}_milestone_{args.milestone}.npy'), samples)
